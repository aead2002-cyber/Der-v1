...
{
    public sealed record DepartmentWriteResult(bool Success, string? Error = null, Dictionary<string, object?>? Item = null);
    ...
    public sealed class DepartmentService : IDepartmentService
    {
        ...
        public async Task<DepartmentWriteResult> CreateAsync(CreateDepartmentRequestDto request, CancellationToken cancellationToken)
        {
            if (IncidentService.HasUnknownFields(request.UnknownFields))
            {
                return new DepartmentWriteResult(false, "Payload contains unsupported fields");
            }

            if (string.IsNullOrWhiteSpace(request.NameAr) || string.IsNullOrWhiteSpace(request.NameEn))
            {
                return new DepartmentWriteResult(false, "nameAr and nameEn are required");
            }
            ...
            var item = await _departmentRepository.InsertAsync(department, cancellationToken);
            return new DepartmentWriteResult(true, Item: item);
        }

        public async Task<DepartmentWriteResult> UpdateAsync(string id, UpdateDepartmentRequestDto request, CancellationToken cancellationToken)
        {
            ...
            if (request.Id.HasValue)
            {
                ...
                {
                    return new DepartmentWriteResult(false, "Route id and body id must match");
                }
            }
            ...
            var item = await _departmentRepository.UpdateAsync(trimmedId, fields, cancellationToken);
            return item is null
