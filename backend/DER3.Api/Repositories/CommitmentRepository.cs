...
namespace DER3.Api.Repositories
{
    ...
    public sealed class CommitmentRepository : ICommitmentRepository
    {
        ...
        public async Task<Dictionary<string, object?>?> UpdateAsync(string id, IReadOnlyDictionary<string, object?> fields, CancellationToken cancellationToken)
        {
            await using var connection = await OpenConnectionAsync(cancellationToken);
            await using var command = connection.CreateCommand();
            command.CommandText = $"UPDATE Commitment SET {string.Join(", ", fields.Keys.Select((key, index) => $"[{key}] = @p{index}"))} WHERE id = @id";
        ...
        public async Task<bool> DeleteAsync(string id, CancellationToken cancellationToken)
        {
            await using var connection = await OpenConnectionAsync(cancellationToken);
            await using var command = connection.CreateCommand();
            command.CommandText = "DELETE FROM Commitment WHERE id = @id";
            AddNVarChar(command, "@id", 64, id);
        ...
        {
            await using var command = connection.CreateCommand();
            command.CommandText = "SELECT id, nameAr, nameEn, descriptionAr, descriptionEn, expiryDate, responsibleUser, status, evidenceTitle, evidenceLink, evidenceUploadedAt, createdAt, updatedAt FROM Commitment WHERE id = @id";
            AddNVarChar(command, "@id", 64, id);
