...
namespace DER3.Api.Repositories
{
    ...
    public sealed class NotificationLogRepository : INotificationLogRepository
    {
        ...
        public async Task<Dictionary<string, object?>?> UpdateAsync(string id, IReadOnlyDictionary<string, object?> fields, CancellationToken cancellationToken)
        {
            await using var connection = await OpenConnectionAsync(cancellationToken);
            await using var command = connection.CreateCommand();
            command.CommandText = $"UPDATE NotificationLog SET {string.Join(", ", fields.Keys.Select((key, index) => $"[{key}] = @p{index}"))} WHERE id = @id";
        ...
        public async Task<bool> DeleteAsync(string id, CancellationToken cancellationToken)
        {
            await using var connection = await OpenConnectionAsync(cancellationToken);
            await using var command = connection.CreateCommand();
            command.CommandText = "DELETE FROM NotificationLog WHERE id = @id";
            AddNVarChar(command, "@id", 64, id);
        ...
        {
            await using var command = connection.CreateCommand();
            command.CommandText = "SELECT id, recipientId, recipientEmail, recipientName, type, subject, body, status, sentAt, errorMessage FROM NotificationLog WHERE id = @id";
            AddNVarChar(command, "@id", 64, id);
