...
export function AttachmentsField({ value, onChange, label, multiple = true }: AttachmentsFieldProps) {
  ...
  const labelText = label || (isRtl ? 'Ø§Ù„Ù…Ø±ÙÙ‚Ø§Øª' : 'Attachments');
  ...
  const handleUpload = async (files: FileList | null) => {
    ...
    for (const file of Array.from(files)) {
      try {
        ...
        toast.error(isRtl ? `İÔá ÑİÚ: ${file.name}` : `Upload failed: ${file.name}`);
      }
    }
    setUploading(false);
    onChange(next);
  ...
  const open = (val: string) => {
    ...
    if (url) {
      filesApi.openFile(val).catch(() => toast.error(isRtl ? 'ÊÚĞÑ İÊÍ ÇáãÑİŞ' : 'Could not open attachment'));
    }
  };
    ...
    <div className="space-y-2">
      ...
      <div className="flex flex-wrap gap-2 items-center">
        ...
        <label className={cn(
          ...
          {isRtl ? 'Ø¥Ø¶Ø§ÙØ© Ù…Ø±ÙÙ‚' : 'Add Attachment'}
          <input
            type="file"
            multiple={multiple}
            className="hidden"
