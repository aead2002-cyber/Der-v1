  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-text-main">{isRtl ? 'إدارة المخاطر' : 'Risk Management'}</h1>
          <p className="text-text-muted mt-1">{isRtl ? 'تسجيل وتقييم المخاطر وربطها بالإجراءات' : 'Register, assess, and link risks to procedures'}</p>
        </div>
      ...
      <Dialog open={!!deleteConfirmId} onOpenChange={(v) => { if (!v) setDeleteConfirmId(null); }}>
        <DialogContent className="sm:max-w-[400px]">
          ...
          <p className="py-4 text-text-main font-medium">{isRtl ? 'هل أنت متأكد من حذف هذا الخطر؟' : 'Delete this risk?'}</p>
          <DialogFooter className="gap-2">
            <Button variant="ghost" onClick={() => setDeleteConfirmId(null)}>{t('cancel')}</Button>
            <Button onClick={handleDelete} className="bg-rose-600 hover:bg-rose-700 text-white font-bold px-6">{t('delete')}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
