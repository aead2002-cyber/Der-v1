...
export function PermissionGroupsManager() {
  ...
  const handleDelete = () => {
    ...
    if (ok) {
      ...
      toast.error(isRtl ? 'لا يمكن حذف المجموعات الافتراضية' : 'Built-in groups cannot be deleted');
    }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h3 className="text-lg font-bold text-text-main flex items-center gap-2">
            ...
            {isRtl ? 'المجموعات والصلاحيات' : 'Groups & Permissions'}
          </h3>
          <p className="text-[12px] text-text-muted mt-1">
            {isRtl
              ? 'تعريف مجموعات الصلاحيات وتعيين الصلاحيات لكل مجموعة. يُمكن إسناد كل مستخدم إلى مجموعة واحدة.'
            ...
            <div key={g.id} className="bg-white rounded-2xl border border-border-subtle shadow-sm p-5 flex flex-col gap-3">
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="inline-flex items-center rounded-lg border border-border-subtle bg-white shadow-sm overflow-hidden divide-x divide-border-subtle rtl:divide-x-reverse">
                  ...
                  <button
                    type="button"
                    title={g.isSystem ? (isRtl ? 'لا يمكن حذف المجموعات الافتراضية' : 'Built-in groups cannot be deleted') : t('delete')}
              ...
              <div className="text-[11px] text-text-muted font-medium">
                {g.permissions.length} / {totalKeys} {isRtl ? 'صلاحية' : 'permissions'}
              </div>
        ...
        <DialogContent className="sm:max-w-[920px] max-h-[90vh] overflow-hidden flex flex-col rounded-2xl border-none shadow-2xl" dir={isRtl ? 'rtl' : 'ltr'}>
          <DialogHeader>
            <DialogTitle className="text-lg font-bold flex items-center gap-2">
              <Shield className="w-5 h-5 text-primary" />
              {editing?.isSystem
                ? (isRtl ? 'تعديل صلاحيات مجموعة افتراضية' : 'Edit Built-in Group')
                : editing && groups.some(g => g.id === editing.id)
                  ? (isRtl ? 'تعديل مجموعة' : 'Edit Group')
                  : (isRtl ? 'إضافة مجموعة' : 'Add Group')}
            </DialogTitle>
          </DialogHeader>
          ...
          {editing && (
            <div className="flex-1 overflow-y-auto -mx-1 px-1 space-y-5">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-1.5">
                  <label className="text-[12px] font-bold text-text-main">{isRtl ? 'الاسم بالإنجليزي' : 'English Name'} <span className="text-red-500">*</span></label>
                  <Input
      ...
      <Dialog open={!!confirmDelete} onOpenChange={v => { if (!v) setConfirmDelete(null); }}>
        <DialogContent className="sm:max-w-[420px] rounded-2xl border-none shadow-2xl" dir={isRtl ? 'rtl' : 'ltr'}>
          <DialogHeader>
            <DialogTitle className="text-lg font-bold">{isRtl ? 'تأكيد الحذف' : 'Confirm Delete'}</DialogTitle>
          </DialogHeader>
          <p className="text-[13px] text-text-muted leading-relaxed">
            {isRtl
              ? `هل أنت متأكد من حذف المجموعة "${confirmDelete?.nameAr || ''}"؟ المستخدمون المعيّنون لها سيفقدون صلاحياتهم.`
              ...
              <div className="bg-slate-50 px-4 py-2.5 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <button
                    ...
                    title={allSvc ? (isRtl ? 'إلغاء الكل' : 'Unselect all') : (isRtl ? 'تحديد الكل' : 'Select all')}
                  >
