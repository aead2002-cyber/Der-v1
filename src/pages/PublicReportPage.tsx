...
const PublicReportPage: React.FC = () => {
  ...
  const handleHumanCheck = () => {
    ...
    setTimeout(() => {
      if (failed) {
        toast.error(isRtl ? 'تعذر التحقق. حاول مرة أخرى.' : 'Verification failed. Please try again.');
  ...
  // every time the user attempts to navigate back â€” keeping them on this page.
  useEffect(() => {
  ...
  const handleSubmit = async () => {
    ...
    if (!humanVerified) {
      toast.error(isRtl ? 'يرجى تأكيد أنك لست روبوتًا' : 'Please confirm you are not a robot');
      return;
    }
                      ...
                      <span className={cn('text-sm font-bold', humanVerified ? 'text-emerald-700' : 'text-slate-700')}>
                        {humanVerified
                          ...
                          : verifyingHuman
                            ...
                            : (isRtl ? 'أنا لست روبوتًا' : "I'm not a robot")}
                      </span>
                    </button>
