...
export default function LoginPage() {
  ...
  const cardTitle = t('login', { defaultValue: isRtl ? 'تسجيل الدخول' : 'Sign In' });
  const cardSubtitle = t('login_subtitle', {
    defaultValue: isRtl
      ? 'يرجى إدخال بريدك الإلكتروني وكلمة المرور للوصول إلى المنصات الداخلية.'
      : 'Please use your email and password to access the internal platforms.',
  });
  ...
  const handleSendOTP = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.ok) {
        setStep('otp');
      toast.success(t('code_sent_to', { email }));
    } else {
      toast.error(result.error || t('invalid_email_or_password', { defaultValue: isRtl ? 'البريد الإلكتروني أو كلمة المرور غير صحيحة' : 'Invalid email or password' }));
    }
  } catch (error) {
    toast.error(error instanceof Error ? error.message : t('login_failed', { defaultValue: isRtl ? 'فشل تسجيل الدخول' : 'Login failed' }));
  } finally {
    setLoading(false);
  }
  };
  ...
  const handleForgotPassword = async (e: React.FormEvent) => {
    ...
    toast.success(t('password_reset_link_sent', { defaultValue: isRtl ? 'تم إرسال رابط إعادة التعيين إلى بريدك الإلكتروني' : 'Password reset link sent to your email' }));
      setIsForgotPassOpen(false);
      setResetEmail('');
    } catch (error) {
    toast.error(t('failed_to_send_reset_link', { defaultValue: isRtl ? 'فشل في إرسال رابط إعادة التعيين' : 'Failed to send reset link' }));
    } finally {
  ...
  const handleVerifyOTP = async (e: React.FormEvent) => {
    ...
    toast.success(t('login_successful', { defaultValue: isRtl ? 'تم تسجيل الدخول بنجاح' : 'Login successful' }));
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-[#F8FAFC] px-4 font-sans" dir={isRtl ? 'rtl' : 'ltr'}>
      ...
      <button
        ...
        title={t('change_language', { defaultValue: isRtl ? 'تغيير اللغة' : 'Change language' })}
      >
        {isRtl ? t('english', { defaultValue: 'English' }) : t('arabic', { defaultValue: 'العربية' })}
      </button>
                    ...
                    <div className="space-y-4">
                      <div className="space-y-2">
                        <Label className="mb-1 block text-sm font-bold text-[#0B3768]">
                          {t('email', { defaultValue: isRtl ? 'البريد الإلكتروني' : 'Email' })} <span className="text-[#6F8E3D]">*</span>
                        </Label>
                      ...
                      <div className="space-y-2">
                        <Label className="mb-1 block text-sm font-bold text-[#0B3768]">
                          {t('password', { defaultValue: isRtl ? 'كلمة المرور' : 'Password' })} <span className="text-[#6F8E3D]">*</span>
                        </Label>
                        <div className="relative">
                          <Lock className="absolute left-3 top-3 h-4 w-4 text-[#0B3768]/70 rtl:left-auto rtl:right-3" />
                          <Input
                            type={showPassword ? 'text' : 'password'}
                            placeholder={t('password', { defaultValue: isRtl ? 'كلمة المرور' : 'Password' })}
                            value={password}
                        ...
                        <div className="flex justify-start">
                          <Link
                            to="/report-incident"
                            className="text-sm font-semibold text-[#0B3768] underline-offset-4 transition-colors hover:text-[#6F8E3D] hover:underline"
                          >
                            {t('submit_report_without_login', { defaultValue: isRtl ? 'إرسال بلاغ بدون تسجيل دخول' : 'Submit report without login' })}
                          </Link>
                        </div>
                      </div>
                    </div>
                  ...
                  >
                    <div className="space-y-2">
                      ...
                      <p className="text-center text-[11px] text-[#0B3768]/75">
                        {t('check_email_for_code', { defaultValue: isRtl ? 'تحقق من بريدك الإلكتروني للحصول على الرمز' : 'Check your email for the verification code' })}
                      </p>
                    </div>
              ...
              <div className="space-y-4">
                <div className="grid gap-2">
                  <Label className="text-[11px] font-black uppercase tracking-widest text-[#0B3768]">
                    {t('email', { defaultValue: isRtl ? 'البريد الإلكتروني' : 'Email' })}
                  </Label>
                  <div className="relative">
