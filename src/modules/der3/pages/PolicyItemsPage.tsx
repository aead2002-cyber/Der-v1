...
import { getPolicyItemProgress, getStandardItemIds } from '@/lib/progressHelpers';
import { ExportMenu } from '@/modules/der3/components/ExportMenu';
import { useTableSort } from '@/modules/der3/components/useTableSort';
import { SortableTh } from '@/modules/der3/components/SortableTh';
import { PolicyItemsImport } from '@/modules/der3/components/PolicyItemsImport';
import { Pagination, usePagination } from '@/modules/der3/components/Pagination';
import { AccordionView } from '@/modules/der3/components/PolicyItemsAccordion';
import { PolicyItemFormDialog } from '@/modules/der3/components/PolicyItemFormDialog';
import { PolicyItem, Policy, Standard, Framework, Procedure } from '@/types';
import { toast } from 'sonner';
import { cn } from '@/lib/utils';
import { useAuth } from '@/AuthContext';
import { frameworksApi } from '@/modules/der3/services/frameworksApi';
...
export default function PolicyItemsPage() {
  ...
  const loadData = async () => {
    try {
      const [itemRows, policyRows, frameworkRows, standardRows, procedureRows] = await Promise.all([
  ...
  const sortedItems = sort
    ? sortedRows(filteredItems, (it: any, key: string) => {
        switch (key) {
          ...
          case 'progress': return getPolicyItemProgress(it.id, standards, procedures);
  ...
  return (
    <div className="space-y-8">
      <PageHeader
        title={t('policy_items')}
        description={t('manage_policy_items_desc')}
        actions={
          ...
          <div className="inline-flex rounded-lg border border-border-subtle bg-slate-50/50 p-1">
            <button
              ...
              title={isRtl ? 'ط¹ط±ط¶ ط¬ط¯ظˆظ„' : 'Table view'}
            >
            ...
            <button
              ...
              title={isRtl ? 'ط¹ط±ط¶ ط£ظƒظˆط±ط¯ظٹظˆظ†' : 'Accordion view'}
            >
              <LayoutList className="w-4 h-4" />
              {isRtl ? 'ط£ظƒظˆط±ط¯ظٹظˆظ†' : 'Accordion'}
            </button>
          </div>
          <PolicyItemsImport onDone={loadData} />
          <ExportMenu
            title={isRtl ? 'ط§ظ„ط¨ظ†ظˆط¯' : 'Policy Items'}
            filename="policy_items"
            rows={filteredItems}
            columns={[
              { header: isRtl ? 'ط§ظ„ط§ط³ظ… ط¨ط§ظ„ط¹ط±ط¨ظٹ' : 'Name (Arabic)', accessor: (i: any) => i.nameAr },
              { header: isRtl ? 'ط§ظ„ط§ط³ظ… ط¨ط§ظ„ط¥ظ†ط¬ظ„ظٹط²ظٹ' : 'Name (English)', accessor: (i: any) => i.nameEn },
              { header: isRtl ? 'ط§ظ„ط³ظٹط§ط³ط©' : 'Policy', accessor: (i: any) => getPolicyName(i.policyId) },
              { header: isRtl ? 'ط§ظ„ط¨ظ†ط¯ ط§ظ„ط£ط¨' : 'Parent', accessor: (i: any) => i.parentId ? (items.find(x => x.id === i.parentId)?.[isRtl ? 'nameAr' : 'nameEn'] || '') : '' },
              { header: isRtl ? 'ط§ظ„طھط±طھظٹط¨' : 'Order', accessor: (i: any) => i.order ?? 0 },
              { header: isRtl ? 'ط§ظ„ظ…ط¹ط§ظٹظٹط± ط§ظ„ظ…ط±طھط¨ط·ط©' : 'Linked Standards', accessor: (i: any) => standards.filter((s: any) => getStandardItemIds(s).includes(i.id)).length },
              { header: isRtl ? 'ظ†ط³ط¨ط© ط§ظ„ط§ظ„طھط²ط§ظ… %' : 'Progress %', accessor: (i: any) => `${getPolicyItemProgress(i.id, standards, procedures)}%` },
              { header: isRtl ? 'ط§ظ„ظˆطµظپ ط¨ط§ظ„ط¹ط±ط¨ظٹ' : 'Description (AR)', accessor: (i: any) => i.descriptionAr || '' },
              { header: isRtl ? 'ط§ظ„ظˆطµظپ ط¨ط§ظ„ط¥ظ†ط¬ظ„ظٹط²ظٹ' : 'Description (EN)', accessor: (i: any) => i.descriptionEn || '' }
            ]}
          />
            ...
            <div className="flex flex-wrap gap-3 items-end">
              <div className="space-y-1.5 flex-1 md:flex-none">
                <label className="text-[11px] font-bold text-indigo-600 uppercase tracking-wider px-1 flex items-center gap-1">
                  ...
                  {t('framework') || (isRtl ? 'ط¥ط·ط§ط± ط§ظ„ط¹ظ…ظ„' : 'Framework')}
                </label>
                      ...
                      <div className="absolute z-50 mt-1 w-full bg-white border border-border-subtle rounded-lg shadow-lg max-h-[300px] overflow-y-auto">
                        ...
                        {filtered.length === 0 ? (
                          <div className="px-3 py-3 text-[12px] text-text-muted text-center italic">
                            {isRtl ? 'ظ„ط§ طھظˆط¬ط¯ ظ†طھط§ط¦ط¬' : 'No results'}
                          </div>
                        ) : filtered.map(p => (
                          <button
                            key={p.id}
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr>
                ...
                <SortableTh sortKey="framework" sort={sort} onToggle={toggle} className={cn(isRtl ? "text-right" : "text-left")}>{t('framework') || (isRtl ? 'ط¥ط·ط§ط± ط§ظ„ط¹ظ…ظ„' : 'Framework')}</SortableTh>
                        ...
                        >
                          ...
                          <div className="mt-1.5 inline-flex items-center gap-1 px-2 py-0.5 bg-blue-50/60 border border-blue-100 rounded-md w-fit">
                            ...
                            <span className="text-[10px] font-bold text-blue-700">
                              {stdCount} {isRtl ? 'ظ…ط¹ظٹط§ط± ظ…ط±طھط¨ط·' : 'linked standards'}
                            </span>
                          </div>
                        </div>
                      </td>
                        ...
                        <div className="flex items-center justify-center">
                          <div className="inline-flex items-center rounded-lg border border-border-subtle bg-white shadow-sm overflow-hidden divide-x divide-border-subtle rtl:divide-x-reverse">
                            {can('policies.policy_items.create') && (
                              <button
                                type="button"
                                title={isRtl ? 'ط¥ط¶ط§ظپط© ط¨ظ†ط¯ ظپط±ط¹ظٹ' : 'Add sub-item'}
                                onClick={() => {
                            ...
                            {can('policies.policy_items.link_standards') && (
                              <button
                                type="button"
                                title={isRtl ? 'ط±ط¨ط· ظ…ط¹ط§ظٹظٹط±' : 'Link standards'}
                                onClick={() => {
        ...
        <DialogContent className="sm:max-w-[560px] rounded-xl border-none shadow-2xl">
          <DialogHeader>
            <DialogTitle className="text-xl font-bold flex items-center gap-2">
              ...
              {isRtl ? 'ط±ط¨ط· ظ…ط¹ط§ظٹظٹط± ط¨ط§ظ„ط¨ظ†ط¯' : 'Link Standards to Item'}
            </DialogTitle>
          </DialogHeader>
          {linkDialogItem && (() => {
            ...
            return (
              <div className="space-y-4">
                ...
                <div className="text-[12px] text-text-muted">
                  {isRtl
                    ? `ط§ط®طھط± ط§ظ„ظ…ط¹ط§ظٹظٹط± ط§ظ„طھظٹ طھط±ط؛ط¨ ط¨ط±ط¨ط·ظ‡ط§ ط¨ظ‡ط°ط§ ط§ظ„ط¨ظ†ط¯ ط¶ظ…ظ† ط§ظ„ط³ظٹط§ط³ط© (${policyStandards.length} ظ…ط¹ظٹط§ط± ظ…طھط§ط­)`
                ...
                <div className="max-h-[340px] overflow-y-auto space-y-1 border border-border-subtle rounded-lg p-1.5">
                  {policyStandards.length === 0 ? (
                    <div className="text-center text-[12px] text-text-muted py-8">
                      {isRtl ? 'ظ„ط§ طھظˆط¬ط¯ ظ…ط¹ط§ظٹظٹط± ظپظٹ ظ‡ط°ظ‡ ط§ظ„ط³ظٹط§ط³ط©' : 'No standards in this policy'}
                    </div>
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {otherLinks.length > 0 && (
                            <div className="text-[10px] text-slate-500 mt-0.5">
                              {isRtl ? `ظ…ط±طھط¨ط· ط£ظٹط¶ط§ظ‹ ط¨ظ€ ${otherLinks.length} ط¨ظ†ط¯ ط¢ط®ط±` : `Also linked to ${otherLinks.length} other item(s)`}
                            </div>
                          )}
                        </div>
                        <div className={cn(
          ...
          <DialogFooter className="gap-2">
            ...
            >
              ...
              {isRtl ? 'ط­ظپط¸ ط§ظ„ط±ط¨ط·' : 'Save Links'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
        ...
        <DialogContent className="sm:max-w-[500px] rounded-xl border-none shadow-2xl">
          <DialogHeader>
            <DialogTitle className="text-xl font-bold">
              {isRtl ? 'ط¥ط¶ط§ظپط© ط¨ظ†ط¯ ظپط±ط¹ظٹ ط³ط±ظٹط¹' : 'Quick Add Sub-item'}
            </DialogTitle>
          </DialogHeader>
      ...
      <ConfirmDialog
        ...
        description={t('confirm_delete_policy_item_desc') || 'Are you sure you want to delete this policy item? All related standards will also be affected.'}
